...
Reqable offers a diverse range of data filters, specifically: [Bookmark](#bookmark), [Device](#device), [Application](#application), [Domain](#domain), [Quick Filter](#quick-filter), and [Search Filter](#search-filter). Among these, `Bookmark`, `Device`, `Application`, and `Domain` are located in the [Explorer](./explorer) sidebar, while `Quick Filter` and `Search` are situated at the top of the traffic list.
...
Reqable includes a built-in "My Bookmark" folder by default; you can also create your own custom bookmark folders.
...
- Right click to pin the device to the top and keep it there.
:::
...
The domains are also used to quickly filter traffic, and can be used together with bookmarks using OR logic. All domains in the list will be displayed in the explorer. You can select one or more domains for filtering. If not selected, the list will display all domains.
...
The logical relationship between each set of filter criteria defaults to "AND". However, you can click the icon shown in the image above to switch the logical relationship to "OR." Clicking the "Save Search Filter" icon in the image allows you to save your current filter settings. Saved configs appear in the [Quick Filter](#quick-filter), allowing you to apply them directly with a single click in the future.
