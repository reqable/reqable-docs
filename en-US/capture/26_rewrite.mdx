...
In addition, redirection does not block the request sent to the original address server, but actually just replaces the response data of the original address server.
...
A request or response can be rewritten multiple rules (by creating different rules to match the same url), which will inevitably involve the issue of execution sequence. We can adjust the order in the list of rewriting rules (shortcut key <Shortcut>Control + ↓/↑</Shortcut>) to set the priority, and the first one in the list will be executed first.

### Import and Export

Reqable supports batch import and export of rewrite rules, which is convenient for sharing and collaboration with others.
