...
Reqable can replay one or more requests. Select the requests that need to be replayed and right-click:
...
- Iterations: The number of times the request was sent.
...
- Proxy: You can set the proxy configuration for the request, including Unset, Follow Debug, and Follow System.

For example:
```
Jack set a replay request https://www.test.com/get at 13:00:00,
Iterations is 3 times, the delay is 3000ms, and the interval is 10000ms.
The time point of sending the first request is 13:00:03, with a delay of 3s.
The time point of sending the second request is 13:00:13, which is 10s after the previous request.
The time point when the third request is sent is 13:00:23, which is 20s after the first request starts.
```
...
- If you want the request to appear in the traffic list, please set the proxy to **Follow Debug**.
