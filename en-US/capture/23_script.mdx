...
The function receives two parameters [CaptureContext](addons#api-context) and [CaptureHttpRequest](addons#api-request) and returns [CaptureHttpRequest](addons#api-request).
...
The function receives two parameters [CaptureContext](addons#api-context) and [CaptureHttpResponse](addons#api-response) and returns [CaptureHttpResponse](addons#api-response).
...
A request or response can use multiple scripts (by creating different rules to match the same url), which will inevitably involve the issue of the order in which the scripts are executed. We can adjust the sequence in the script list (shortcut key <Shortcut>Control + ↓/↑</Shortcut>) to set the priority, and the first one in the list will be executed first.
