...
Script is one of the core debugging features of Reqable, which supports writing Python scripts to process HTTP requests and responses. Reqable provides a set of Python-based APIs. In scripts, we can modify, replace or delete the request and response packets, and even interrupt the request and response. In addition to using Reqable's built-in API, users can also use various Python third-party packages, such as `requests` and so on.

You can enable the script via the menu path `Traffic` -> `Script` -> `Enable` (Shortcut key <Shortcut>Shift + Control + P</Shortcut>). Once the script is enabled, an activation icon will appear on the toolbar.

![](arts/script_01.png)

Click the icon above to disable the script, once disabled, the icon will disappear from the toolbar. If you wish to keep the script icon permanently visible on the toolbar, you can right-click the toolbar to edit and select to pin it.
...
You can create a script rule in several ways:
- Traffic menu -> Script -> Add Rule
- Script icon right-click menu -> Add Rule
- Tray -> Script -> Add Rule
- Script List -> New
- Script List -> Right-Click Menu -> New
- Traffic list -> Select an item -> Right-click menu -> Script
...
If you need to interrupt a response, the result of the onResponse function returns `None`!
...
Reqable provides a debug console for script logs and error backtraces. Keep the script editor window open during script execution, and all information will be output to the console in real time.
...
The script function depends on the local Python3 environment, please make sure that Python3 is installed on the local device. For beginners, please refer to [Python downloads](https://www.python.org/downloads/).
