...
Gateway is one of the core debugging features of Reqable. As the name suggests, it means network gateway. Note that the gateway here is different from the gateway in computer terms, and it only means the unique statement about traffic control in Reqable.
...
You can create a gateway rule in several ways:
...
- Traffic list -> Select an item -> Right-click menu -> Gateway
