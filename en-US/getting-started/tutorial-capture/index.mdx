...
First, you need to enable the debugging, and tap the `Start` button in the toolbar to enter the debugging mode. In the debugging mode, any HTTP request passing through Reqable's MITM proxy server will be displayed on the traffic list.
...
Open a browser, enter the address `https://reqable.com` and go, here uses the Chrome. Back to Reqable, requests from the website have been captured in the traffic list.
...
Tap the `Explorer` icon in the sidebar to open the explorer panel of the traffic list. Then expand the structure tree of `reqable.com`, you can view the directory structure of all requests under this website, and select to view the details of the request.
