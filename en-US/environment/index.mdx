---
title: Environments
description: Get familiar with the concept and basic usage of environment variables.
sidebar_position: 4
---
...
Environments in Reqable are divided into two types: `Global Environment` and `User Environment`. Variables in `Global Environment` are available throughout the Reqable application, while `User Environment` variables are set for a specific environment (such as development, testing, production, etc.).

By using environments, users can easily switch between different environments without having to manually change values in each request. Open Reqable's `Environment` panel to create and manage environments and set different variable values for each environment. These environment variables are then used in requests to dynamically reference these values based on the currently active environment.
