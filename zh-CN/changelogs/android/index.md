...
- 💪【优化】文件缓存从 /Android/data 迁移到 /data/data。
- 🐞【修复】纠正一些英文拼写和语法错误。
...
- 💪【优化】重做抓包数据历史记录时间设置页面。
- 🐞【修复】桌面端应用介绍中图片加载失败的bug。
- 🐞【修复】域名分组的请求列表项长按菜单中功能失效的bug。
...
- 💪【优化】默认以文本模式而不是表格模式显示数据。
- 💪【优化】调试列表搜索会忽略超过16M的数据防止OOM。
- 💪【优化】64位设备支持16k page size对齐。
...
- 🐞【修复】标签页长按选项和Tooltip冲突的bug。
- 🐞【修复】文本编辑器使用输入法的剪贴板功能粘贴多行文本会显示异常的bug。
- 🐞【修复】历史记录页面Tab未正确切换的bug。
- 🐞【修复】版本更新提示被底部导航栏遮挡的bug。
...
- 💪【优化】调试列表数据导出的文件命名规则。
- 💪【优化】用户证书安装指引的文案描述更加准确。
- 🐞【修复】从API集合中删除API时未能自动关闭API标签页的bug。
- 🐞【修复】API测试请求头名称使用环境变量提示非法名称的bug。
- 🐞【修复】Android网络请求堆栈信息未保存到历史记录中的bug。
- 🐞【修复】协同二维码显示无有效IP地址的bug。
- 🐞【修复】英文语言下Android官方文档链接拼写错误的bug。
- 🐞【修复】打开应用列表可能出现闪退的bug。
- 🐞【修复】API集合页面按返回键后未回到上级目录而是退到首页的bug。
