...
- 💪【优化】多语言国际化支持繁体中文、日语、韩语、西班牙语、法语、德语和俄语。
- 💪【优化】设置中添加检测到版本更新是否进行提示的选项。
- 💪【优化】添加DNS缓存选项。
- 💪【优化】重写整体替换时不再向服务器发送请求。
- 💪【优化】网关屏蔽请求和挂起请求移除WebSocket类型。
- 💪【优化】版本更新下载成功后自动安装。
- 💪【优化】全屏时禁止拖动窗口顶部。
- 🐞【修复】HEAD请求未忽略响应体的bug。
- 🐞【修复】图片解码失败时标签显示不正确的bug。

## v3.2.16 <small><small>*2026-07-28*</small></small>
- 💪【优化】导出cURL提供保留环境变量选项。
- 💪【优化】cURL导入时不再校验URL是否规范。
- 💪【优化】只读型JSON将使用序列化的方式格式化。
- 💪【优化】自动识别XML格式的数据并自动格式化。
- 💪【优化】从调试列表创建重写、断点和脚本规则时自动带入请求方法。
- 💪【优化】提升清理历史记录的通知的准确性和按钮文案。
- 💪【优化】自动推断的抓包数据类型可以被快捷筛选栏过滤。
- 🐞【修复】某些类型的响应一直显示加载中状态的bug。

## v3.2.15 <small><small>*2026-07-27*</small></small>
- 💪【优化】降低抓包列表数据过多时出现的网络延迟。
- 💪【优化】调整子窗口默认尺寸。
- 💪【优化】调试列表标签页标题中抓包记录数最多显示9999。
- 💪【优化】[MCP] 快捷栏添加MCP服务器启用状态。
- 💪【优化】[MCP] 支持查看哪些MCP客户端启动并连接了MCP服务器。
- 💪【优化】[MCP] 新增Trae、Qoder、CodeBuddy配置指引。
- 🐞【修复】纠正一些英文拼写和语法错误。
- 🐞【修复】JSON压缩未正确处理转义字符的bug。
- 🐞【修复】生成cURL功能，当请求头中不包含Content-Type时，错误使用 `-d` 而不是 `--data-raw` 的bug。
- 🐞【修复】导入cURL功能，当请求头中不包含Content-Type时，错误使用form表单导入而不是raw导入。
...
- 💪【优化】取消系统代理时自动恢复之前的配置。
- 💪【优化】抓包应用信息获取更加准确。
- 💪【优化】API请求测试标签页右键菜单添加保存和另存选项。
- 💪【优化】解决调试列表部分性能问题。
- 💪【优化】导出分部内容使用正确的默认文件名称。
- 💪【优化】[MCP] 超过256k的数据返回临时文件路径而不是直接返回内容。
- 💪【优化】[MCP] 版本升级时自动关闭Server进程。
- 🐞【修复】抓包详情中请求头部复制为JSON时未合并Cookie的bug。
- 🐞【修复】[MCP] 获取抓包列表ID不正确bug。
- 🐞【修复】调试列表按住Ctrl键向上选择时列表会自动滚动的bug。
- 🐞【修复】窗口顶部无法调整窗口尺寸的bug。
...
- 💪【优化】[MCP] Content将返回JSON结构数据而不是提示文案。
...
- 💪【优化】SSE支持配置自定义提取器。
- 💪【优化】[MCP] 抓包数据详情中添加完整的连接信息。
- 💪【优化】[MCP] 抓包数据详情中添加应用信息。
- 💪【优化】JWT工具解析结果中时间戳支持右键格式化。
...
- 💪【优化】[MCP] 默认仅注册常用工具，如需注册全部工具，需使用参数`--scope all`指定。
- 💪【优化】[MCP] 完善MCP服务器配置指引。
- 💪【优化】[MCP] 提供控制是否覆盖系统代理的Tool。
- 🐞【修复】[MCP] 无法无法控制开始和关闭抓包bug。

## v3.2.5 <small><small>*2026-07-09*</small></small>
- 💪【优化】cURL支持设置多行模式并默认采用多行模式。
- 💪【优化】cURL支持设置使用单引号还是双引号并默认采用单引号。
- 💪【优化】socks5代理支持UDP。
- 💪【优化】调试列表请求未完成时URL的Scheme不再使用代理协议名称。
- 💪【优化】报告服务器URL添加语法高亮和自动补全功能。
- 🐞【修复】某些情况下代理服务器可能无法连接的bug。
- 🐞【修复】API测试中请求体内存泄露的bug。
- 🐞【修复】代理端口出现冲突并自动+1后添加远程设备无法连接的bug。
- 🐞【修复】报告服务器规则无法禁用的bug。
- 🐞【修复】Android证书安装ADB扫描不到设备一直显示Loading的bug。
- 🐞【修复】请求测试侧边历史记录复制URL和复制cURL时未解析环境变量的bug。

## v3.2.4 <small><small>*2026-07-02*</small></small>
- 💪【优化】API请求测试历史记录支持设置保存时间。
- 💪【优化】API请求测试历史记录支持记住时间聚合模式还是URL聚合模式。
- 💪【优化】API请求测试历史记录支持按分组删除。
- 💪【优化】功能要求高级会员时出现一个友好的Toast提示。
- 💪【优化】[MCP] 修改部分Tool要求登录的提示文案。
- 💪【优化】`代理`菜单中新增修改端口号入口。
- 🐞【修复】鼠标滚轮无法滚动横向标签页的bug。
- 🐞【修复】重启应用后已经被删除的历史记录仍然会显示在列表中的bug。
- 🐞【修复】集合导出界面无法滚动到底部的bug。
- 🐞【修复】环境变量导出界面无法滚动到底部的bug。
- 🐞【修复】API测试历史记录双击打开不会固定标签页的bug。
- 🐞【修复】新建收藏文件夹时未验证可用数量的bug。

## v3.2.3 <small><small>*2026-06-30*</small></small>
- 💪【优化】[MCP] 从调试列表请求生成的cURL直接返回而不是通过结构化输出。
- 💪【优化】[MCP] 配置指引文案。
- 💪【优化】[MCP] 新增一个从API集合生成cURL的Tool。
- 💪【优化】[MCP] 调试列表请求数据会将脚本日志提供给AI。
- 🐞【修复】[MCP] 无法自动获取到用户自定义代理端口的bug。
- 🐞【修复】[MCP] 当Server路径中存在空格时，按指引配置到Cursor无法启动的bug。

## v3.2.2 <small><small>*2026-06-30*</small></small>
- 💪【优化】Text视图中保存的请求体和响应体将直接保存二进制数据而不是使用utf8重新编码。
- 💪【优化】Antigravity的MCP指引改成通用指引。
- 💪【优化】MCP配置指引中命令行安装场景路径使用单`\`。
- 🐞【修复】部分MCP配置指引中JSON文件路径错误的bug。
- 🐞【修复】字体文件未正常打包导致字体可能显示异常的bug。

## v3.2.1 <small><small>*2026-06-29*</small></small>
- 🐞【修复】代理端口号未被占用时仍然可能自动+1的bug。

## v3.2.0 <small><small>*2026-06-29*</small></small>
- 🚀【新功能】开放Reqable各项功能控制接口。
- 🚀【新功能】提供官方MCP支持。
- 💪【优化】重新登录账户后将从云端全量同步数据，避免出现云端与本地数据不一致的问题。
- 💪【优化】规则名称长度限制由32调整到64。
- 💪【优化】环境变量添加`reqablePort`内置变量，表示当前代理端口。
- 💪【优化】镜像规则字段添加名称。
- 💪【优化】新建各项规则时不再自动开启相关功能。
- 💪【优化】调试列表URL搜索默认匹配解码的数据无需手动启用。
- 🐞【修复】WebSocket测试无法从调试列表创建的bug。
- 🐞【修复】WebSocket测试无法从调试列表添加到集合的bug。
- 🐞【修复】307和308重定向将请求方法改为GET并丢弃请求体的bug。
- 🐞【修复】SOCKS代理相关选项导致cURL导入失败的bug。
- 🐞【修复】WebSocket请求生成代码会打开HTTP请求生成代码的bug。
- 🐞【修复】脚本编辑器控制台当显示时间戳时复制内容可能不正确的bug。
- 🐞【修复】调试历史记录双击打开不会固定标签页的bug。

## v3.1.3 <small><small>*2026-05-29*</small></small>
- 💪【优化】更新Android证书安装Magisk模块。
- 💪【优化】Base64工具中解码二进制结果可以直接打开图片查看器进行查看。
- 💪【优化】当有弹窗遮盖时禁用下层文件拖拽功能。
- 💪【优化】可执行文件版本号不再包含构建版本号。
- 💪【优化】应用重启后可以正常恢复之前窗口最大化的状态。
- 🐞【修复】SSL不兼容旧版重协商机制的bug。
- 🐞【修复】部分语言生成代码中URL未正确编码的bug。
- 🐞【修复】数字域名无法解析的bug。
- 🐞【修复】设置中清理缓存未能删干净的bug。
- 🐞【修复】设置中清理缓存会导致用户调试脚本被删除的bug。
- 🐞【修复】API授权未能正确跳转到继承授权页面的bug。
- 🐞【修复】导入部分ApiPost集合文件时URL未正确获取的bug。
...
- 🐞【修复】代理终端右键切换终端程序时配置异常的bug。

## v3.1.1 <small><small>*2026-05-10*</small></small>
- 💪【优化】代理终端和设备二维码图标合并到快捷栏。
- 💪【优化】添加自定义快捷键提示图标。
...
- 🐞【修复】重启应用后反向代理可能运行失败的bug。
- 🐞【修复】反向代理请求可能出现无响应的bug。
...
- 🐞【修复】调试侧边栏中可能出现未知应用跳动的问题。
- 🐞【修复】重写删除请求参数预览数据显示不正确的bug。
...
- 🐞【修复】API集合某些情况下不会自动定位到当前打开的API的bug。
- 🐞【修复】API集合`Delete`快捷键不生效的bug。
- 🐞【修复】右键上下文菜单点击无效项会关闭整个菜单的bug。

## v3.0.37 <small><small>*2026-01-22*</small></small>
- 💪【优化】历史记录过多时会在右下角显示通知提示清理。
- 🐞【修复】HTTP访问本地服务器的端口号解析可能不正确的bug。
- 🐞【修复】窗口最大化顶部显示溢出的bug。

## v3.0.36 <small><small>*2026-01-20*</small></small>
- 💪【优化】更新CA证书下载链接。
- 💪【优化】支持导入Insomnia的Yaml集合数据。
- 💪【优化】导入集合或者环境后自动切换到的新导入的环境。
- 💪【优化】已是高级会员时AES工具图标不再显示付费标记。
- 🐞【修复】导入Reqable格式集合会丢失WebSocket请求的bug。
- 🐞【修复】高分辨率显示器上字体显示异常问题。

## v3.0.35 <small><small>*2026-01-19*</small></small>
- 💪【优化】urlencode请求文本模式下支持输入并解析`foo=bar&abc=123`格式。
- 💪【优化】主页右上角环境变量图标使用标签色。
- 🐞【修复】登录状态下使用离线数据模式时脚本内容加载不正确的bug。

## v3.0.34 <small><small>*2026-01-16*</small></small>
- 💪【优化】生成代码支持Dart-Dio。
- 💪【优化】JSON Tree视图会自动记住上一次的搜索类型。
- 💪【优化】优先使用用户设置的ADB路径而不是使用环境变量。
- 💪【优化】兼容ADB 36.0.0和36.0.1版本推送文件最后一个字符被截断的bug，这会导致一键安装证书失败。
- 💪【优化】抓包总览Tab中显示应用程序进程ID。
- 💪【优化】脚本Context中添加TCP连接信息。
- 💪【优化】JSON查看器添加打开文件和另存为按钮。
- 🐞【修复】首页标签页前后滚动按钮的状态可能不正常的bug。
- 🐞【修复】ADB路径设置后无法再取消设置的bug。
- 🐞【修复】脚本执行期间切换环境变量会导致变量更新到错误环境的bug。
...
- 💪【优化】Hash工具自动加载上一次的配置。
- 💪【优化】HMAC工具自动加载上一次的配置。
- 💪【优化】HMAC工具调整密钥和输入内容的位置。
- 💪【优化】工具菜单和右键上下文菜单中添加HMAC工具入口。
- 💪【优化】AES工具解密自动去除密文中的换行符。
- 💪【优化】AES工具自动加载上一次的配置并会自动执行。
- 💪【优化】时间戳工具自动加载上一次的配置。
- 💪【优化】开放ADB路径设置入口。
...
- 🐞【修复】代理终端中运行python-requests请求出现SSL验证报错的bug。
...
- 🐞【修复】缺失`Content-Length`的请求无法执行重写、断点和脚本的bug。
- 🐞【修复】API测试历史记录搜索结果排序错乱的bug。
- 🐞【修复】配置备份文件损坏可能导致应用无法正常启动的bug。

## v3.0.30 <small><small>*2025-12-08*</small></small>
- 🚀【新功能】新增Java VM证书自动安装功能。
- 💪【优化】form-data请求分部不再发送Content-Length头部。
- 🐞【修复】点击窗口关闭按钮可能无法进入后台模式的bug。
- 🐞【修复】Python脚本中无法使用内置环境变量的bug。
- 🐞【修复】首页`+`创建的API未应用全局设置的bug。
...
- 💪【优化】支持批量导出API集合。
- 💪【优化】支持批量导出环境变量。
- 💪【优化】支持将文件拖拽进集合视图中导入数据。
- 💪【优化】支持一次性导入多个集合文件。
- 💪【优化】支持将文件拖拽进环境变量视图中导入数据。
- 💪【优化】支持一次性导入多个环境变量文件。
- 💪【优化】首页支持拖拽打开多个文件。
- 💪【优化】首页标签页宽度设置了一个最大值。
- 💪【优化】API脚本执行报错时控制台显示其他已执行脚本的输出信息。
- 💪【优化】加载历史记录可能导致应用启动变慢的问题。
- 💪【优化】统一部分弹窗按钮顺序和文案。
- 💪【优化】HAR标签页上下文菜单添加刷新选项。
- 🐞【修复】Python脚本中使用新构建的Query执行报错的bug。
- 🐞【修复】Digest授权未正确处理环境变量的bug。

## v3.0.28 <small><small>*2025-11-28*</small></small>
- 💪【优化】更换cURL命令解析器。
- 💪【优化】导入cURL时发现文件不存不再阻止导入。
- 💪【优化】form-data增加对文件不存在的警告提醒。
- 💪【优化】兼容一些非标准的`content-type`类型。
- 💪【优化】首页右上角环境变量显示名称而不是仅仅是图标。
- 💪【优化】首页标签栏右键`+`可以直接创建HTTP请求。
- 💪【优化】调试列表支持状态码条件搜索。
- 💪【优化】脚本支持`request.contentType和response.contentType`赋值修改类型。
- 💪【优化】脚本中断请求时控制台显示相关信息而不是显示报错。
- 🐞【修复】WebSocket和SSE中HEX视图无法获取焦点导致快捷键无效的bug。
- 🐞【修复】调试列表中SSE视图由于数据编码问题导致数据不显示的bug。
- 🐞【修复】XML美化未能正确处理内嵌`>`和`<`的bug。
- 🐞【修复】导入cURL单引号和双引号处理不正确的bug。
- 🐞【修复】脚本编辑器可能出现卡死的问题。
- 🐞【修复】脚本编辑器`编辑中`和`已保存`状态可能显示不正确的bug。
- 🐞【修复】API请求脚本执行后未清理残余文件的问题。
- 🐞【修复】从编辑数据中新建环境变量弹窗标题显示的环境名称不正确的bug。
- 🐞【修复】代理终端中运行node或者依赖node的命令报错的bug。

## v3.0.27 <small><small>*2025-11-18*</small></small>
- 💪【优化】版本更新时显示全部的新版本日志。
- 💪【优化】IP列表显示VPN地址。
- 💪【优化】自动清理过时的日志文件。
- 💪【优化】支持打开新版Charles的.chlz格式文件。
- 💪【优化】设置中禅模式选项位置前移。
- 💪【优化】设置中新增快速创建HTTP选项控制首页`+`按钮点击后直接创建HTTP请求。
- 🐞【修复】导入cURL空值请求头时未去除`;`的bug。
- 🐞【修复】从文件关联启动应用不会自动打开文件的bug。
- 🐞【修复】API请求标签页保存提示状态在某些情况下显示不正确的bug。
- 🐞【修复】导入Postman集合未能正确识别请求体类型的bug。

## v3.0.26 <small><small>*2025-11-14*</small></small>
- 💪【优化】完善从HTTP原始报文创建API的逻辑。
- 💪【优化】cURL导入内容输入框自动获取焦点。
- 💪【优化】cURL导入自动过滤首尾的空格和换行符。
- 💪【优化】API调试表单请求支持一次性选择多个文件。
- 💪【优化】API调试表单请求体直接显示切换类型入口而不是输入数据后再显示。
...
- 🐞【修复】从v2.32.0之前版本升级到新版本后二级文件夹数据丢失的bug。

## v3.0.23 <small><small>*2025-11-11*</small></small>
- 💪【优化】启动应用时检测是否为版本降级并显示进行提醒。
- 💪【优化】设置中添加`表格模式`视图控制选项。
- 🐞【修复】API授权引用环境变量不生效的bug。
- 🐞【修复】API授权引切换类型不生效的bug。
- 🐞【修复】WebSocket请求头部名称被强制转成小写的bug。
- 🐞【修复】自定义主题色设置无效的bug。
- 🐞【修复】重写处理Query时进行重复编码的bug。
- 🐞【修复】导入Reqable V2版本的API集合丢失文件夹配置的bug。

## v3.0.22 <small><small>*2025-11-06*</small></small>
- 💪【优化】便捷化登录注册操作步骤。
- 💪【优化】生成代码PHP添加对curl_close的兼容判断。
- 💪【优化】右下角自动弹出许可证迁移通知。
- 💪【优化】表格模式编辑时可以使用`Shift + Tab`向前切换输入框。
- 🐞【修复】导入集合数据时JSON中的`{{`和`}}`会被错误替换成`<<`和`>>`的bug。
- 🐞【修复】日志文件被删除后打开日志窗口灰屏的bug。
...
- 🚀【新功能】支持SVG图片预览功能。
- 💪【优化】标签页右键菜单支持向右关闭。
- 💪【优化】重写重定向的URL显示时将省略根路径`/`。
- 💪【优化】查看请求历史不再弹窗提示是否`还原`数据。
- 💪【优化】支持导入Yaml格式的OpenAPI集合。
- 💪【优化】请求和响应面板显示关闭和折叠图标按钮。
- 💪【优化】URL自动补全scheme由https改为http。
- 💪【优化】cURL导入自动处理JSON转移字符。
- 💪【优化】完善环境变量名称错误提示信息。
- 💪【优化】生成代码中Node fetch使用动态导入。
- 💪【优化】生成代码中axios输出响应体而不是整个响应对象。
- 🐞【修复】重写重定向可能导致其他数据修改功能失效的bug。
- 🐞【修复】响应的重写拦截器在历史记录和新窗口中不显示的bug。
- 🐞【修复】导入集合数据时JSON中的`{{`和`}}`会被错误替换成`<<`和`>>`的bug。
- 🐞【修复】导入HAR格式集合数据时未正确处理HTTP2头部的bug。
- 🐞【修复】导入OpenAPI集合时错误导入Components的bug。
- 🐞【修复】部分规则列表搜索未输入关键词时可能会显示上一次搜索结果的bug。
- 🐞【修复】环境变量鼠标悬浮提示中变量值可能未更新的bug。
- 🐞【修复】被禁用调整尺寸的窗口可以被最大化的bug。
- 🐞【修复】应用重启之后窗口位置和大小无法恢复原先状态的bug。
- 🐞【修复】代理终端无法拦截Nodejs fetch发送的请求的bug。
- 🐞【修复】代理终端拦截axios发送的HTTPS请求无响应的bug。
- 🐞【修复】重写帮助文档网页路径跳转不正确的bug。
- 🐞【修复】关闭新创建且未编辑的API也会提示进行保存的bug。
- 🐞【修复】API请求urlencode编码方式不正确的bug。
- 🐞【修复】API请求自定义代理设置未正确保存的bug。
- 🐞【修复】API请求修改请求体时未及时更新标签页修改状态的bug。
- 🐞【修复】API请求重定向未能正确处理`//`起始的Location的bug。
- 🐞【修复】API请求全局设置部分场景不生效的bug。
- 🐞【修复】API请求继承的Basic Auth中环境变量未解析的bug。
- 🐞【修复】API请求Basic Auth中环境变量在切换当前环境后未生效的bug。
- 🐞【修复】Hoppscotch环境变量导入后丢失值的bug。
- 🐞【修复】不规范的Cookie可能导致应用卡死的bug。
- 🐞【修复】修复Cookie新建和编辑不生效的bug。
- 🐞【修复】HAR文件解析不到记录的bug。
- 🐞【修复】文本编辑器搜索匹配内容可能会被遮挡的bug。
- 🐞【修复】HTTP2协议的请求在某些情况下失败的bug。
- 🐞【修复】调试列表选中项并添加到新会话导致列表被清空的bug。
- 🐞【修复】HexViewer搜索输入非法正则会导致搜索一直显示无结果的bug。
...
- 💪【优化】SSE支持切换正序显示和逆序显示。
- 💪【优化】WebSocket支持切换正序显示和逆序显示。
- 💪【优化】首页标签Tab鼠标悬浮显示完整标题。
- 🐞【修复】WebSocket切换搜索和过滤后已阅状态不正确的bug。
- 🐞【修复】301、302和303重定向时未将新地址的请求方法改为GET的bug。
- 🐞【修复】图标按钮在连续点击时可能无法触发点击事件的bug。
- 🐞【修复】重写重定向路径`*`替换可能失效的bug。
- 🐞【修复】调试工具日志控制台窗口中无法打开日志目录的bug。

## v2.33.10 <small><small>*2025-04-21*</small></small>
- 💪【优化】MIME为非文本类型时，数据将不会进行字符编码检测。
- 💪【优化】API测试关闭`跟随调试`后切换到`不设置`而不是`跟随系统`。
- 💪【优化】Python脚本支持使用`[]`操作Binary和Multipart类型数据。
- 💪【优化】删除`导出`菜单无效的快捷键提示文案。
- 💪【优化】Swagger导入数据的URL默认使用http而不是https。
- 🐞【修复】Hex标签下保存的数据可能不是显示数据的bug。

## v2.33.9 <small><small>*2025-04-15*</small></small>
- 💪【优化】API请求头部复制为JSON功能自动移除内置的空值头部。
- 🐞【修复】导出的Postman文件未标明https的bug。
- 🐞【修复】顶级API集合目录可以被拖拽移动到其他集合子文件夹内的bug。
- 🐞【修复】脚本编辑器控制台输出复制功能异常bug。

## v2.33.8 <small><small>*2025-04-02*</small></small>
- 💪【优化】WebSocket和SSE帧详细数据自动格式化。
- 💪【优化】表格视图支持使用Tab键切换焦点单元格。
- 💪【优化】生成代码复制不再自动关闭弹窗。
- 🐞【修复】从调试列表创建form-data请求未自动删除原始`Content-Type`头部的bug。
- 🐞【修复】导入CA证书后SSL证书缓存未清理的bug。
- 🐞【修复】某些请求已经完成但状态显示为Processing或者Abort的bug。
- 🐞【修复】API导出cURL未正确处理JSON注释的bug。
- 🐞【修复】多窗口状态下关闭主窗口可能导致程序闪退的bug。
...
- 💪【优化】检测Python环境时，优先检测`python.exe`而不是`python3.exe`。
- 💪【优化】启用后台模式时，`Alt + F4`将进入后台而不是退出程序。
...
- 🚀【新功能】支持在新窗口中打开二进制。
- 🚀【新功能】支持在新窗口中打开API响应。
- 🚀【新功能】API测试支持使用快捷键`Control + Shift + I`从剪贴板快速导入cURL。
- 🚀【新功能】API测试支持使用快捷键`Control + Shift + E`生成cURL并写入剪贴板。
