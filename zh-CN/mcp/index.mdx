...
在AI助手中配置好后，便可以让AI操控本机的Reqable了。

如果要让AI控制非本机（例如手机）的Reqable，在配置MCP时可以使用 `--host` 和 `--port`参数。

| 参数 | 缩写 | 说明 | 默认值 |
| --- | --- | --- | --- |
| `--host` | `-h` | Reqable API 服务地址。 | `127.0.0.1` |
| `--port` | `-p` | Reqable API 服务端口。 | 优先读取 Reqable 本地配置中的代理端口号，读取失败时回退到 `9000` |

### 支持功能

Reqable MCP服务器目前提供了140+的Tool，按功能分类如下。
