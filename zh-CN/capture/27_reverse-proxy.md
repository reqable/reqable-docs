...
Reqable支持创建本地Web服务器，透明地将流量代理到远程服务器，同时Reqable将捕获这些流量并支持[重写](./rewrite)、[断点](./breakpoint)和[脚本](./script)等调试功能。
...
Reqable支持配置多个反向代理并进行管理，可以从应用`调试`菜单中的`反向代理`创建和管理反向代理规则。
